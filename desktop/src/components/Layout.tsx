    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex">
      ...
      >
        <div className={`flex items-center gap-3 border-b border-slate-800/80 ${collapsed ? 'px-4 py-5 justify-center' : 'px-5 py-5'}`}>
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-700 to-amber-900 flex items-center justify-center shadow-lg shadow-amber-900/40 shrink-0">
            <svg className="w-5 h-5 text-amber-100" viewBox="0 0 24 24" fill="currentColor">
          ...
          {!collapsed && (
            <div className="min-w-0">
              <p className="font-semibold text-sm leading-tight truncate text-slate-100">Exportadora Romex</p>
              <p className="text-[11px] text-amber-500/90 truncate">Cadmio & Plaguicidas</p>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-6">
          {NAV.map((group) => {
            const items = group.items.filter((i: any) => !i.adminOnly || user.role === 'ADMIN');
        ...
        <div className="border-t border-slate-800/80 p-3 space-y-2">
          {!collapsed && (
            <div className="px-2 py-2 rounded-xl bg-slate-900/60">
              ...
              <p className="text-[10px] text-amber-400/90 mt-0.5">{user.role || '—'}</p>
            </div>
          )}
