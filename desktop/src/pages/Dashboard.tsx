...
export default function Dashboard() {
  ...
  // Tiempo real
  useSocket(
    () => { loadData(); }, // sample updated
    () => { loadData(); }, // sample created
  );
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm">Resumen general del comportamiento del cadmio</p>
        </div>
        {lastUpdate && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <Kpi title="Promedio Cadmio" value={stats?.cadmium?.avg != null ? `${Number(stats.cadmium.avg).toFixed(3)} ppm` : '-'} />
        <Kpi title="Máximo Cadmio" value={stats?.cadmium?.max != null ? `${Number(stats.cadmium.max).toFixed(3)} ppm` : '-'} color="text-red-400" />
        <Kpi title="Mínimo Cadmio" value={stats?.cadmium?.min != null ? `${Number(stats.cadmium.min).toFixed(3)} ppm` : '-'} color="text-green-400" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          ...
          {productData.length > 0 ? (
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie data={productData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90}
